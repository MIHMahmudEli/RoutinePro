    ...
    try {
        ...
        if (loginRes.status === 200) {
            ...
            if (lBody.includes('CaptchaInputText') || lBody.includes('CaptchaDeText')) {
                if (lBody.toLowerCase().includes('invalid') || lBody.toLowerCase().includes('incorrect') || lBody.toLowerCase().includes('error')) {
                    throw new Error('Invalid credentials or CAPTCHA — please reload CAPTCHA and try again');
                }
                throw new Error('Login failed — still on login page. Check credentials and CAPTCHA.');
            }
        }
