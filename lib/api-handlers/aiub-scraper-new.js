...
async function scrapeAIUBCourses(username, password, captchaText, csrfToken, captchaDeText, initialCookies) {
    try {
        ...
        if (!rawCourses) {
            ...
            // Try to extract semester name from the page
            // Example: <h5 class="panel-title"> Offered Sections - 2025-2026, Summer </h5>
            const semesterMatch = cBody.match(/Offered Sections\s*-\s*([0-9]{4}-[0-9]{4},\s*[A-Za-z]+|[A-Za-z]+\s+[0-9]{4}-[0-9]{4})/i);
            if (semesterMatch && semesterMatch[1]) {
                let rawSem = semesterMatch[1].trim();
                if (rawSem.includes(',')) {
                    const parts = rawSem.split(',').map(s => s.trim());
                    scrapedSemester = `${parts[1].toUpperCase()} ${parts[0]}`;
                } else {
                    scrapedSemester = rawSem.toUpperCase();
                }
                console.log(`[AIUB] Extracted semester: ${scrapedSemester}`);
            }
        }
