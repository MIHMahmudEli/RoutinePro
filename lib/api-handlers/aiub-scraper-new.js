...
async function fetchCaptchaFromAIUB() {
    try {
        ...
        // Fetch CAPTCHA image using the SAME cleaned cookies from the loginPageRes and same-origin headers
        const imgRes = await fetchWithCookies(captchaUrl, 'GET', null, {
...
async function scrapeAIUBCourses(username, password, captchaText, csrfToken, captchaDeText, initialCookies) {
    try {
        ...
        if (!cookiesStr) {
            ...
            cookiesStr = cleanCookies(loginPageRes.cookies);
        }
        ...
        const newCleanCookies = cleanCookies(loginRes.cookies);
        const mergedCookies = [cookiesStr, newCleanCookies].filter(Boolean).join('; ');
