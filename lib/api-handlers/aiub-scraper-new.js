...
function parseCoursesFromHTML(html) {
    ...
    // Scan every <tr> in the full HTML — avoids nested-table regex issues
    ...
    while ((rowMatch = rowRegex.exec(html)) !== null) {
        ...
        // Need at least 10 cells; ClassID must be numeric; title must exist
        if (cells.length < 10) continue;
        if (!cells[0] || !/^\d+$/.test(cells[0])) continue;  // ClassID must be a number
        if (!cells[1]) continue;                               // Title must be present
