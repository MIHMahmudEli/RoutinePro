...
async function fetchWithCookies(url, method = 'GET', body = null, headers = {}, cookies = '') {
...
async function fetchCaptchaFromAIUB(cookies) {
    try {
        console.log('[AIUB] Fetching login page for CAPTCHA...');
        const loginPageRes = await fetchWithCookies('https://portal.aiub.edu/Login', 'GET', null, {}, cookies.join(';'));
        ...
        // Extract CAPTCHA image URL from various patterns
        let captchaUrl = null;
        const imgPattern = /<img[^>]*src=["']([^"']*(?:Captcha|captcha)[^"']*)["']/i;
        const imgMatch = loginPageRes.body.match(imgPattern);
        if (imgMatch) captchaUrl = imgMatch[1];

        if (!captchaUrl) {
            captchaUrl = '/DefaultCaptcha/Generate';
        }

        if (captchaUrl && !captchaUrl.startsWith('http')) {
            captchaUrl = 'https://portal.aiub.edu' + (captchaUrl.startsWith('/') ? captchaUrl : '/' + captchaUrl);
        }

        console.log('[AIUB] Fetching CAPTCHA from:', captchaUrl);

        // Fetch CAPTCHA image
        const imgRes = await fetchWithCookies(captchaUrl, 'GET', null, {}, cookies.join(';'));
        
        if (imgRes.status === 200) {
            const base64 = Buffer.from(imgRes.body, 'binary').toString('base64');
...
async function scrapeAIUBCourses(username, password, captchaText, csrfToken, initialCookies) {
    try {
...
export default async function handler(request, response) {
    ...
    const { username, password, captcha, csrfToken, action, cookies } = request.body;

    // Fetch CAPTCHA
    if (action === 'fetch-captcha') {
        try {
            const loginPageRes = await fetchWithCookies('https://portal.aiub.edu/Login', 'GET');
            const result = await fetchCaptchaFromAIUB(loginPageRes.cookies);
    ...
    try {
        const result = await scrapeAIUBCourses(username, password, captcha, csrfToken, cookies);
